...
const char EffectCharsMulti[] = {
  0xaa, 0xaa, 0xbd, 0xbd, 0xbd, 0xbd, 0x55, 0x55,
  0x00, 0x3c, 0xff, 0xff, 0xff, 0xff, 0x3c, 0x00, 
...
const char screenStartX = BOWLSTARTX / BLOCKSIZE;
const char screenStartY = BOWLSTARTY / BLOCKSIZE;
...
static char* nextTileFlicker[4];
static char nextTileFlickerCount = 0;
void showHideNextTile(bool hide) {
  ...
  for(char y = 0; y < 2; y++) {
    //#pragma unroll(full)
    for(char x = 0; x < 4; x++) {
      Screen[(40 * (PREVIEWY)) + (40 * y) + (PREVIEWX + 1 + x)] = CHAR_EMPTY;
    }
  }
  while(nextTileFlickerCount > 0) {
    removeOrange(nextTileFlicker[nextTileFlickerCount - 1]);
    nextTileFlickerCount--;
  }
  if(! hide) {

    // draw the shape of the next tile
    char c = 0;
    char tester = TheGame.nextTile.color;
    Screen[160] = tester + 48;
    for(char y = 0; y < TheGame.nextTile.dimy[0]; y++) {
      for(char x = 0; x < TheGame.nextTile.dimx[0]; x++) {
        if(TheGame.nextTile.data[c] == 1) {
          char screenpos = (40 * (PREVIEWY))+ (40 * y) + (PREVIEWX + 1 + x);
          
          if(false && TheGame.nextTile.color == VCOL_ORANGE) {
            /*
            nextTileFlicker[nextTileFlickerCount] = &Color[screenpos];
            addOrange(nextTileFlicker[nextTileFlickerCount]);
            nextTileFlickerCount++;
            */
            Screen[screenpos] = 134;
            Color[screenpos] = VCOL_RED;
          } else {
            Screen[screenpos] = singleBlockCharacter;
            Color[screenpos] = TheGame.nextTile.color;
          }
        }
        c++;
      }
...
bool rotate(bool left) {
  ...
  char oldRow = TheGame.tile_row;
  char oldLine = TheGame.tile_line;
  
  // switch to SRS / wall kick rotation style
  if(try_rotate(&tempRot, TheGame.tileno, &TheGame.tile_row, &TheGame.tile_line, left ? -1 : 1)) {
    fixSpriteOffsets(tempRot);
    TheGame.rotation = tempRot;
    if(oldRow != TheGame.tile_row) {
      char diffX = TheGame.tile_row_graph - ((oldRow * BLOCKSIZE) << FBITS);
      TheGame.tile_row_graph = ((TheGame.tile_row * BLOCKSIZE) << FBITS) + diffX;
    }
    if(oldLine != TheGame.tile_line) {
      char diffY = TheGame.tile_line_graph - ((oldLine * BLOCKSIZE) << FBITS);
      TheGame.tile_line_graph = ((TheGame.tile_line * BLOCKSIZE) << FBITS) + diffY;
    }
    moveTileToPos(false);
    drawShadow();
    return true;
...
void redrawGrid(void) {
  ...
  for(char y = 0; y < 20; y++) {
    //#pragma unroll(full)
    for(char x = 0; x < 10; x++) {
      int screenPos = 40 * (screenStartY + y) + screenStartX + x;
      if(TheGame.grid[x][y] > 0) {
        if(false && tiles[TheGame.grid[x][y] - 1].color == VCOL_ORANGE) {
          Screen[screenPos] = 134;
          Color[screenPos] = VCOL_RED;
        } else {
          Screen[screenPos] = singleBlockCharacter;
      ...
      if(where > 0) {
        for(signed char y1 = (where - 1); y1 >= 0; y1--) {
          for(char x2 = 0; x2 < 10; x2++) {
            /*
            if(tiles[TheGame.grid[x2][y1] - 1].color == VCOL_ORANGE) {
              removeOrange(&Color[(screenStartY * 40) + ((y1 - 1) * 40) + screenStartX + x2]);
              addOrange(&Color[(screenStartY * 40) + (y1 * 40) + screenStartX + x2]);
            }
            */
...
void moveToNewPos(void) {
  ...
  if(TheGame.sidemove != 0) {
    ...
    if (sideCollision) {
      ...
      // TODO: make the amount of pixels level / speed dependant
  ...
  if(bottomCollision) {
    ...
    for(char x = 0; x < TheGame.currentTile.dimx[TheGame.rotation]; x++) {
      for(char y = 0; y < TheGame.currentTile.dimy[TheGame.rotation]; y++) {
        if(TheGame.currentTile.data[TheGame.currentTile.accessNodes[TheGame.rotation] + TheGame.currentTile.dimx[TheGame.rotation] * y + x] == 1) {
          ...
          /*
          if(TheGame.currentTile.color == VCOL_ORANGE) {
            organgePos[fl] = &Color[(screenStartY * 40) + ((TheGame.tile_line + y) * 40) + screenStartX + TheGame.tile_row + x];
            fl++;
          }
          */
        }
    ...
    // set added orange screen positions to flicker to orange
    /*
    while(fl > 0) {
      addOrange(organgePos[fl]);
      fl--;
    }
    */
...
int main(void)
{
  ...
  //oscar_expand_lzo(Charset, MicroFont);
  //memcpy(Charset, MicroFont, 2048);
  memcpy(Charset, Charset + 2048, 2048);
  memcpy(Charset + CHAR_EFFECT_FADE1 * 8, EffectChars, sizeof(EffectCharsMulti));
  generate_scroll_chars();
  mmap_set(MMAP_NO_BASIC);
  //music_init(0);
  memcpy(Sprite, SpriteImage, 64);
  //memcpy(Sprite + 64, OrangeSpriteImage, 64);
  ...
  memset(Screen, CHAR_EMPTY, 25 * 40);
  char startX = (BOWLSTARTX / 8) - 1;
  char startY = BOWLSTARTY / 8;

  // draw the tetris bowl
  for(char y = 0; y < 20; y++) {
    Screen[40 * (startY + y) + startX] = CHAR_BOWL;
    Screen[40 * (startY + y) + startX + 11] = CHAR_BOWL;
    Color[40 * (startY + y) + startX] = 13;
    Color[40 * (startY + y) + startX + 11] = 13;
  }
  for(char x = 0; x < 12; x++) {
    Screen[40 * (startY + 20) + startX + x] = CHAR_BOWL;
    Color[40 * (startY + 20) + startX + x] = 13;
  }
  
  // draw the lines and the preview field
  for(char x = 0; x < 40; x++) {
    Screen[(PREVIEWY - 1) * 40 + x] = CHAR_LINE_HORIZONTAL;
    Color[(PREVIEWY - 1) * 40 + x] = VCOL_LT_GREY;
  }
  Screen[(PREVIEWY - 1) * 40  + PREVIEWX] = CHAR_LINE_BRANCH_DOWN; 
  Screen[(PREVIEWY - 1) * 40  + PREVIEWX + 5] = CHAR_LINE_BRANCH_DOWN;
  Color[(PREVIEWY - 1) * 40  + PREVIEWX] = VCOL_LT_GREY; 
  Color[(PREVIEWY - 1) * 40  + PREVIEWX + 5] = VCOL_LT_GREY;
  Screen[(40 * PREVIEWY) + (40 * 2) + PREVIEWX] = CHAR_LINE_CORNER_LOWER_LEFT; 
  Screen[(40 * PREVIEWY) + (40 * 2) + PREVIEWX + 5] = CHAR_LINE_CORNER_LOWER_RIGHT;
  Color[(40 * PREVIEWY) + (40 * 2) + PREVIEWX] = VCOL_LT_GREY; 
  Color[(40 * PREVIEWY) + (40 * 2) + PREVIEWX + 5] = VCOL_LT_GREY;
  for(char y = 0; y < 2; y++) {
    Screen[(40 * PREVIEWY) + (40 * y) + PREVIEWX] = CHAR_LINE_VERTICAL;
    Screen[(40 * PREVIEWY) + (40 * y) + (PREVIEWX + 5)] = CHAR_LINE_VERTICAL;
    Color[(40 * PREVIEWY) + (40 * y) + PREVIEWX] = VCOL_LT_GREY;
    Color[(40 * PREVIEWY) + (40 * y) + (PREVIEWX + 5)] = VCOL_LT_GREY;
    if(y == 1) {
      for(char x = 0; x < 4; x++) {
        Screen[(40 * PREVIEWY) + (40 * y) + 40 + PREVIEWX + x + 1] = CHAR_LINE_HORIZONTAL;
